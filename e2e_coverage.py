...
def change_base_dir(new_dir: Optional[str] = None) -> None:
    ...
    env.update({
        "RASA_DOMAIN_PATH": f"{env['BOT_PATH']}/domain.yml".replace("//", "/"),
        "STORY_REPORT_PATH": f"{env['BOT_PATH']}/results/story_report.json".replace("//", "/"),
...
def check_directory(botname: str) -> None:
    change_base_dir(f"tests/mocks/{botname}")
